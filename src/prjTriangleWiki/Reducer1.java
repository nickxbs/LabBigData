...
public class Reducer1 extends
	...
	protected void reduce(TextPair key, Iterable<Text> values,Context context) throws IOException, InterruptedException {
		...
		for (Text valText : values) {
			...
			{
				if(!partialJoin.contains(val.toString()))
					partialJoin.add(val.toString());
			}
			else	//link to
			{
	...
	private void WriteContext(Text key, Context context,String value) throws IOException, InterruptedException {
